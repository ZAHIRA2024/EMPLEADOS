...
def destroy(id):
    ...
    cursor.execute("SELECT foto FROM sistema1.empleados WHERE id=%s", id) 
    fila= cursor.fetchall() 
    
    if fila [0]:
        os.remove(os.path.join(app.config['CARPETA'], fila[0][0])) 
    cursor.execute("DELETE FROM sistema1.empleados WHERE id=%s", (id))
    conn.commit()
...
def edit(id):
    ...
    cursor.execute("SELECT * FROM sistema1.empleados WHERE id=%s", (id,))
    empleados = cursor.fetchall()
...
def update():
    ...
    sql = "UPDATE sistema1.empleados SET nombre=%s, correo=%s WHERE id=%s;"
    ...
    if _foto.filename!='': 
        ...
        os.remove(os.path.join(app.config['CARPETA'], fila[0][0]))  
        cursor.execute("UPDATE sistema1.empleados SET foto=%s WHERE id =%s;", (nuevoNombreFoto, id)) 
        conn.commit() 
